...
def turn_traces(spans: list[dict]) -> list[str]:
    """Trace ids of the conversation's turns, in order. run_chat.py opens one chat.turn root per turn."""
    roots = sorted((s for s in spans if s.get("name") == "chat.turn"), key=lambda s: s["startTimeUnixNano"])
    return [root["traceId"] for root in roots]
