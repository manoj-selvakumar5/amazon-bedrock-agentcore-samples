...
This sample mirrors the structure of the sibling [`event-driven-claims-agent`](https://github.com/awslabs/agentcore-samples/tree/main/02-use-cases/02-workflow-automation-agents/event-driven-claims-agent) sample. The first records track decisions shared with that sample (often reaching the same conclusion for the same reason). ADR-0017 onward cover this sample's focus: its evaluators. ADRs 0007, 0010 and 0011 described a model degradation ladder that was removed from this sample ([ADR-0020](0020-remove-degradation-ladder.md)); the numbers are left unused so later references stay valid.
...
| [0008](0008-appconfig-over-hand-rolled-flags.md) | AWS AppConfig for Live Model Settings | Change the model or its inference parameters without a redeploy, with validation + rollback |
...
| [0020](0020-remove-degradation-ladder.md) | Remove the Degradation Ladder, Keep Live Model Settings | Every evaluated run is the same pipeline; the model is still changeable live |
