...
`agentcore.json` is still the single declaration of the AgentCore resources, and the CDK app still renders it with `@aws/agentcore-cdk`. `deploy.sh` now runs the project's own CDK CLI directly (`cdk deploy`) instead of `agentcore deploy`. This removes the dependency on a CLI release that matches the pinned construct version, and synthesis still validates `agentcore.json` against the AgentCore schema. The stack source (`agentcore/cdk/lib/`) is committed through a folder-level `.gitignore` exception; it had been swallowed by the repository's root `lib/` rule.
