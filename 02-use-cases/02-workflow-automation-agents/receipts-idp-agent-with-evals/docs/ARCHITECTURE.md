...
| **Memory** | Integrated in the agent for per-user facts in custom `receipts/...` namespaces (sheddable on lower rungs). The stack does not create a Memory resource by default, and the agent degrades gracefully without one. |
