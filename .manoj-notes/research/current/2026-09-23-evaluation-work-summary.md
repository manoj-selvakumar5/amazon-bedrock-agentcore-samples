...
| `GoalSuccessRate`, after the validator moved to decision tools | 4, each scored twice: the real `send_to_review` trace, a terse but correct concern, a vague concern, the wrong problem named. The extractor's own "does not reconcile" flag stays in every copy | **8 of 8** on a local trace, and **8 of 8** again on a deployed trace (where the text sits in span events, not log records); the extractor's flag did not earn the validator credit |
| `GoalSuccessRate`, is excluding the note writer necessary? | The vague-concern copy of the deployed trace, trimmed and whole; each scored twice | Trimmed: **No, No**. Whole trace: **Yes, Yes**, because the note writer's note names the $8 gap. Without the exclusion the validator would get the note writer's credit |
